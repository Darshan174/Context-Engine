...
const SEV_STYLES = {
  ...
  low:      { pill: "bg-slate-100 dark:bg-black text-slate-500 dark:text-neutral-400", dot: "bg-slate-400" },
};
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5 pb-12">
      ...
      <AgentCard
        ...
        action={<a href="/app/graph" className="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-600 dark:text-neutral-300 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-neutral-700 px-4 py-2 rounded-xl hover:bg-slate-50 dark:hover:bg-black transition-all">Build Graph <ArrowRight className="w-3.5 h-3.5" /></a>}
      />
      ...
      {/* 3 — Gap Detector (HERO) */}
      <div className="rounded-2xl border-2 border-red-200 dark:border-red-900/60 bg-white dark:bg-black shadow-sm overflow-hidden">
      ...
      <AgentCard
        ...
        action={<a href="/app/query" className="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-600 dark:text-neutral-300 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-neutral-700 px-4 py-2 rounded-xl hover:bg-slate-50 dark:hover:bg-black transition-all">Open Ask AI <ArrowRight className="w-3.5 h-3.5" /></a>}
      />
...
function AgentCard({ icon, iconColor, number, title, tagline, examples, action, children }) {
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-neutral-800 bg-white dark:bg-black shadow-sm overflow-hidden">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-slate-500 dark:text-neutral-400">{tagline}</p>
              <div className="flex flex-wrap gap-1.5 mt-2.5">
                {examples.map(ex => (
                  <span key={ex} className="text-[10px] font-medium px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-black text-slate-500 dark:text-neutral-400">{ex}</span>
                ))}
              </div>
            </div>
          </div>
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <div key={s.label} className="rounded-xl bg-slate-50 dark:bg-black border border-slate-100 dark:border-neutral-800/50 p-3 text-center">
        ...
        <div>
          ...
          <div className="space-y-2">
            {report.gaps.map((g, i) => {
              ...
              return (
                <div key={i} className="flex items-start gap-3 p-3.5 rounded-xl bg-slate-50 dark:bg-black border border-slate-100 dark:border-neutral-800/50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-semibold text-slate-800 dark:text-neutral-200">{g.title}</p>
                    <p className="text-xs text-slate-500 dark:text-neutral-400 mt-0.5 leading-relaxed">{g.detail}</p>
                    {g.recommendation && (
...
function RelationshipResult({ report }) {
  return (
    <div className="space-y-4">
      <p className="text-xs text-slate-500 dark:text-neutral-400 italic">{report.message}</p>
        ...
        <div>
          ...
          <div className="space-y-2">
            {report.suggested.map((r, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 dark:bg-black border border-slate-100 dark:border-neutral-800/50">
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-lg shrink-0 mt-0.5 ${r.confidence >= 0.8 ? "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400" : r.confidence >= 0.6 ? "bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400" : "bg-slate-100 dark:bg-black text-slate-500"}`}>
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-neutral-400 mt-0.5">{r.reasoning}</p>
                </div>
              </div>
            ))}
          </div>
...
function ContextPackResult({ pack, copied, onCopy }) {
  return (
    <div>
        ...
        <button
          ...
          className={`flex items-center gap-1.5 text-[11px] font-bold px-3 py-1.5 rounded-lg transition-all ${
            copied
              ...
              : "bg-slate-100 dark:bg-black text-slate-600 dark:text-neutral-300 hover:bg-slate-200 dark:hover:bg-slate-600"
          }`}
        >
      ...
      <pre className="text-[11px] text-slate-600 dark:text-neutral-300 bg-slate-50 dark:bg-black border border-slate-200 dark:border-neutral-800 rounded-xl p-4 overflow-x-auto whitespace-pre-wrap leading-relaxed font-mono max-h-72 overflow-y-auto">
        {pack.content}
      </pre>
    </div>
  );
